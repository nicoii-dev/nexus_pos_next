...
export function LoadingSkeleton({ className, rows = 3, type = "table" }: LoadingSkeletonProps) {
    ...
    return (
      <div className={cn("grid gap-4 md:grid-cols-2 lg:grid-cols-4", className)}>
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="rounded-[10px] border bg-card p-6 shadow-float">
            <div className="flex items-center justify-between">
              <div className="h-4 w-28 rounded-full shimmer" />
              <div className="h-11 w-11 rounded-[10px] shimmer" />
            </div>
            <div className="mt-5">
  ...
  if (type === "card") {
    return (
      <div className={cn("rounded-[10px] border bg-card p-6 shadow-float", className)}>
  ...
  return (
    <div className={cn("rounded-[10px] border bg-card shadow-float", className)}>
      <div className="p-6">
